...
export class RotateImageFileProcessor implements ImageFileProcessor {
  async process(dataURL: string): Promise<string> {
    ...
    let srcOrientation = Number(url[0]);
    let dataURLnew = url[1] + ',' + url[2];
    const image = await createImageFromDataUrl(dataURLnew);
    ...
    ctx.restore();
    return canvas.toDataURL(getImageTypeFromDataUrl(dataURLnew));
  }
}